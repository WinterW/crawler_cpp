...
namespace mongo {
    /* Accessing unaligned doubles on ARM/Linux results in SIGBUS.
       Wrapping in a packed struct forces gcc to generate code that works with unaligned value as well.
    */
#ifdef __arm__
...
#ifdef __arm__
            ((struct packed_double*)grow(sizeof(double)))->d = j;
#else
